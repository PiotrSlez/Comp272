...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        if(node == null){ //makes sure the first node exists
            return;
        }

        if(node.data == oldVal){   //checks if current node is the oldVal
            node.data = newVal;    //replaces old value with newVal
        }

        replaceValueHelper(node.left, oldVal, newVal); //uses recursion to traverse left leaf
        replaceValueHelper(node.right, oldVal, newVal);// traverses right leaf
    }
    ...
    private int findMinHelper(Node node) {
        ...
        //Initially I set min to the max int value, which doesn't work with recursion.

        if(node == null){ //makes sure the first node exists
            return Integer.MAX_VALUE;
        }

        int min = node.data; //sets min to the data of curretn node
        int leftVal = findMinHelper(node.left); //gets value of left leaf
        int rightVal = findMinHelper(node.right); //gets value of right leaf

        if(min > leftVal){ //checks value of leftVal
            min = leftVal; //overwrites min with left node
        }

        if(min > rightVal){ //checks value of rightVal
            min = rightVal; //overwrites min with right node
        }

        return min; //returns the smallest of the three compared nodes, working up the tree
    }
